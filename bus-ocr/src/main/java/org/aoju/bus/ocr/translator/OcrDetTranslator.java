package org.aoju.bus.ocr.translator;
...
import org.aoju.bus.ocr.toolkit.NDArrayKit;
 ...
 *
 */
public class OcrDetTranslator implements Translator<Image, NDList> {
    // det_algorithm == "DB"
    ...
    public OcrDetTranslator(Map<String, ?> arguments) {
        limit_side_len =
    ...
    public NDList processOutput(TranslatorContext ctx, NDList list) {
        ...
        if (this.use_dilation) {
            ...
            Mat srcMat = NDArrayKit.uint8NDArrayToMat(segmentation);
            // size 越小，腐蚀的单位越小，图片越接近原图
            // Mat dilation_kernel = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(2, 2));
            Mat dilation_kernel = NDArrayKit.uint8ArrayToMat(new byte[][]{{1, 1}, {1, 1}});
            /**
    ...
    private NDArray order_points_clockwise(NDArray pts) {
        ...
        return NDArrays.concat(list).reshape(4, 2);
    }
    ...
    private float box_score_fast(NDManager manager, NDArray bitmap, NDArray points) {
        ...
        Mat maskMat = NDArrayKit.uint8NDArrayToMat(mask);

        //mask - convert from NDArray to Mat - 4 rows, 2 cols
        Mat boxMat = NDArrayKit.floatNDArrayToMat(box, CvType.CV_32S);

//        boxMat.reshape(1, new int[]{1, 4, 2});
        List<MatOfPoint> pts = new ArrayList<>();
        MatOfPoint matOfPoint = NDArrayKit.matToMatOfPoint(boxMat); // new MatOfPoint(boxMat);
        pts.add(matOfPoint);
        Imgproc.fillPoly(maskMat, pts, new Scalar(1));


        NDArray subBitMap = bitmap.get(ymin + ":" + (ymax + 1) + "," + xmin + ":" + (xmax + 1));
        Mat bitMapMat = NDArrayKit.floatNDArrayToMat(subBitMap);
