...
public class Solar {
    ...
    public Solar(Date date) {
        Calendar c = Calendar.getInstance(Literal.SOLAR_TIME_ZONE);
        c.setTime(date);
    ...
    public static List<Solar> fromBaZi(String yearGanZhi, String monthGanZhi, String dayGanZhi, String timeGanZhi, int sect, int baseYear) {
        ...
        // 月天干要一致
        if (((Lunar.find(yearGanZhi.substring(0, 1), Literal.LUNAR_GAN, -1) + 1) * 2 + m) % 10 != Lunar.find(monthGanZhi.substring(0, 1), Literal.LUNAR_GAN, -1)) {
            return l;
        }
        // 1年的立春是辛酉，序号57
        int y = Lunar.getJiaZiIndex(yearGanZhi) - 57;
        if (y < 0) {
            y += 60;
        }
        y++;
        ...
        while (y <= endYear) {
            if (y >= startYear) {
                ...
                if (solarTime.getYear() >= baseYear) {
                    // 日干支和节令干支的偏移值
                    int d = Lunar.getJiaZiIndex(dayGanZhi) - Lunar.getJiaZiIndex(solarTime.getLunar().getDayInGanZhiExact2());
                    if (d < 0) {
                        d += 60;
                    }
                    if (d > 0) {
    ...
    public static int getDaysOfMonth(int year, int month) {
        ...
        int m = month - 1;
        int d = Literal.SOLAR_DAYS_OF_MONTH[m];
        //公历闰年2月多一天
    ...
    public static int getWeeksOfMonth(int year, int month, int start) {
        return (int) Math.ceil((getDaysOfMonth(year, month) + fromYmd(year, month, 1).getWeek() - start) * 1D / Literal.SOLAR_WEEK.length);
    }
    ...
    public List<String> getFestivals() {
        ...
        //获取几月几日对应的节日
        String f = Literal.SOLAR_FESTIVAL.get(month + "-" + day);
        if (null != f) {
            l.add(f);
        }
        //计算几月第几个星期几对应的节日
        ...
        //星期几，0代表星期天
        int week = getWeek();
        f = Literal.SOLAR_WEEK_FESTIVAL.get(month + "-" + weeks + "-" + week);
        if (null != f) {
            l.add(f);
        }
        if (day + 7 > getDaysOfMonth(year, month)) {
            f = Literal.SOLAR_WEEK_FESTIVAL.get(month + "-0-" + week);
            if (null != f) {
                l.add(f);
            }
        }
    ...
    public List<String> getOtherFestivals() {
        List<String> l = new ArrayList<String>();
        List<String> fs = Literal.SOLAR_OTHER_FESTIVAL.get(month + "-" + day);
        if (null != fs) {
            l.addAll(fs);
        }
        return l;
    ...
    public String getXingZuo() {
        ...
        return Literal.SOLAR_XINGZUO[index];
    }
    ...
    public int subtract(Solar solar) {
        return getDaysBetween(solar.getYear(), solar.getMonth(), solar.getDay(), year, month, day);
    }
    ...
    public Solar nextMonth(int months) {
        SolarMonth month = SolarMonth.from(year, this.month).next(months);
        int y = month.getYear();
    ...
    public int getSalaryRate() {
        ...
        Holiday holiday = Holiday.getHoliday(year, month, day);
        if (null != holiday) {
            // 法定假日非上班
            if (!holiday.isWork()) {
                return 2;
