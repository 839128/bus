...
public class Optional<T> {
    ...
    /**
     * 获取异常 当调用 {@link #ofTry(XSupplier)}时，异常信息不会抛出，而是保存，调用此方法获取抛出的异常
     *
     * @return 异常
     */
    ...
    /**
     * 是否失败 当调用 {@link #ofTry(XSupplier)}时，抛出异常则表示失败
     *
     * @return 是否失败
     */
    ...
    /**
     * 如果包裹里元素的值存在，就执行对应的操作，并返回本身 如果不存在，返回一个空的{@code Optional} 属于 {@link #ifPresent}的链式拓展
     *
